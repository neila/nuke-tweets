...
def printLike(like):
    print("Liked tweet: " + like['like']['fullText'])
    print("URL: " + like['like']['expandedUrl'])
...
def auth():
    ...
    resource_owner_key = credentials.get('oauth_token')[0] # token and secret must be bytes literal in v2 (b'oauth_token')
...
def removeLike(tweet_id, oauth, user_id):

    try:
        response = oauth.post("https://api.twitter.com/1.1/favorites/destroy.json?id="+tweet_id) # standard v1.1
        # response = requests.delete(url=f'https://api.twitter.com/2/users/{user_id}/likes/{tweet_id}', auth=oauth) # unliking in API v2 uses the delete method but it does not exist in OAuth1Session. Use OAuth1 helper instead.
...
def removeLikes(likes, number_to_remove, oauth, user_id):
    ...
    for like in likes:
        ...
        code = removeLike(like['like']['tweetId'], oauth, user_id)
...
def main():
    ...
    oauth, user_id = auth()
    likes = loadLikes()
    removeLikes(likes, number_to_remove, oauth, user_id)
